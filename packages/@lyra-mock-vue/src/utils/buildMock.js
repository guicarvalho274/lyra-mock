import { ACTIONS, TRANSPORT } from "./_enums.js";
import {
...
/**
 * @typedef {import('./state.js').MockEntry} MockEntry
 * @typedef {import('./state.js').Transport} Transport
 * @typedef {import('./state.js').TransportType} TransportType
 */
 ...
 * @returns {{ type?: TransportType, url?: string | null, delay?: number, error: string | false }}
 */
export function buildTransport({type, url = null, delay = 1000}){

    if(!TRANSPORT.includes(type)){
        return { error: `Error: transport do register precisa estar entre: ${TRANSPORT.join(' ou ')}` }
    }
