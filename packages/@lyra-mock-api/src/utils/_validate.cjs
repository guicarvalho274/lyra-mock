...
//@ts-ignore
const { ACTIONS, SCENARIOS, OPERATORS } = require("./_enums.cjs");

/**
 * @param {unknown} mock
 * @returns {{ index: null, erros: string[] } | null}
 */
function validateMockStructure(mock){
 ...
 * @returns {string | null}
 */
function validateMockIndex(mock){
 ...
 * @returns {string | null}
 */
function validateMockAction(mock){
    if(!ACTIONS.includes(/** @type {any} */ (mock?.action))){
 ...
 * @returns {string | null}
 */
function validateMockGroup(mock){
 ...
 * @returns {string | null}
 */
function validateMockCondition(mock){
 ...
 * @returns {string | null}
 */
function validateConditionItem(mock, prefix = '') {
    ...
    for (let i = 0; i < mock.condition.length; i++) {
        ...
        if (!OPERATORS.includes(/** @type {any} */ (item.operator))) {
 ...
 * @returns {string | null}
 */
function validateScenario(mock){
    if (mock.scenario !== undefined && !SCENARIOS.includes(/** @type {any} */ (mock.scenario))) {
